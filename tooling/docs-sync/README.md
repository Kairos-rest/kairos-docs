...
1. **Cursor** — `state.json` on the VPS holds the last documented `app` `main`
   SHA. Compare that to current `main`; no new commits means no work.
...
`docs_mdx.validate_body` gates every body before it lands:
...
- **Internal vocabulary** — Prisma, Postgres, Redis, Clerk, Vercel, cron,
  ...
  English-only list would miss "migración" and "base de datos".
...
passed, and the PR body says what was dropped and why. Human review is still
the last gate on every PR.

## Files

| File | Purpose |
|---|---|
| `docs-sync.py` | the job: cursor, NAN passes, changelog, commit, PR |
...
~/kairos-docs-pipeline/
  ...
  state.json              cursor: last documented app main SHA
...
| NAN returns non-JSON | treated as "nothing to say", cursor advances, no PR |
